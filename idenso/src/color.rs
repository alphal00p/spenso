...
mod test {
    ...
    fn colored_matrix_element() -> (Atom, Atom) {
        (
            ...
            *g(mink(D,4),mink(D,7))
            *alg::t(coad(alg::Nc^2-1,6),cof(alg::Nc,5),dind(cof(alg::Nc,4)))
            *alg::f(coad(alg::Nc^2-1,7),coad(alg::Nc^2-1,8),coad(alg::Nc^2-1,9))
            *𝟙(bis(D,0),bis(D,5))
            *𝟙(bis(D,1),bis(D,4))
            *𝟙(mink(D,2),mink(D,5))
            *𝟙(mink(D,3),mink(D,6))
            *𝟙(coad(alg::Nc^2-1,2),coad(alg::Nc^2-1,7))
            *𝟙(coad(alg::Nc^2-1,3),coad(alg::Nc^2-1,8))
            *𝟙(coad(alg::Nc^2-1,6),coad(alg::Nc^2-1,9))
            *𝟙(cof(alg::Nc,0),dind(cof(alg::Nc,5)))
            *𝟙(cof(alg::Nc,4),dind(cof(alg::Nc,1)))
            *alg::gamma(mink(D,4),bis(D,5),bis(D,4))
            *alg::vbar(1,bis(D,1))
            *alg::u(0,bis(D,0))
            *alg::ϵbar(2,mink(D,2))
            *alg::ϵbar(3,mink(D,3))",
    ...
    fn test_color_matrix_element() {
        ...
        let spin_sum_rule = parse!(
            "
            𝟙(coad(alg::Nc^2-1, left(3)), coad(alg::Nc^2-1, right(3)))
                * 𝟙(coad(alg::Nc^2-1, left(2)), coad(alg::Nc^2-1, right(2)))
        ...
        let spin_sum_rule_src = parse_lit!(
            alg::vbar(1, bis(D, left(1)))
                * alg::v(1, bis(D, right(1)))
                * alg::u(0, bis(D, left(0)))
                * alg::ubar(0, bis(D, right(0)))
                * alg::ϵbar(2, mink(D, left(2)))
                * alg::ϵ(2, mink(D, right(2)))
        ...
        let spin_sum_rule_trg = parse!(
            ...
            1/4*1/(D-2)^2*
            (
                (-1) * alg::gamma(mink(D,1337),bis(D,left(1)),bis(D,right(1)))*Q(1,mink(D,1337))
                * alg::gamma(mink(D,1338),bis(D,right(0)),bis(D,left(0)))*Q(0,mink(D,1338))
