...
fn symbolic_contract() {
    let structura =
        NamedStructure::from_integers(&[(1, 2), (4, 3)].map(|(a, d)| (a.into(), d.into())), "T");

    let structurb =
        NamedStructure::from_integers(&[(3, 2), (2, 3)].map(|(a, d)| (a.into(), d.into())), "P");
...
fn pres() {
    ...
    let unit = Complex::<f64>::from(1.);
    let mut gamma = SparseTensor::empty(structure);

    gamma.set(&[0, 0, 0], unit).unwrap();
    gamma.set(&[1, 1, 0], unit).unwrap();
    gamma.set(&[2, 2, 0], -unit).unwrap();
    gamma.set(&[3, 3, 0], -unit).unwrap();

    gamma.set(&[0, 3, 1], unit).unwrap();
    gamma.set(&[1, 2, 1], unit).unwrap();
    gamma.set(&[2, 1, 1], -unit).unwrap();
    gamma.set(&[3, 0, 1], -unit).unwrap();

    gamma.set(&[0, 3, 2], -iunit).unwrap();
    gamma.set(&[1, 2, 2], iunit).unwrap();
    gamma.set(&[2, 1, 2], iunit).unwrap();
    gamma.set(&[3, 0, 2], -iunit).unwrap();

    gamma.set(&[0, 2, 3], unit).unwrap();
    gamma.set(&[1, 3, 3], -unit).unwrap();
    gamma.set(&[2, 0, 3], -unit).unwrap();
    gamma.set(&[3, 1, 3], unit).unwrap();
    ...
    println!("{}", paramT.get(&[1, 2]).unwrap());

    for (c, _, _) in gamma.fiber([true, false, true].as_slice().into()).iter() {
        //stuff
    }
}
